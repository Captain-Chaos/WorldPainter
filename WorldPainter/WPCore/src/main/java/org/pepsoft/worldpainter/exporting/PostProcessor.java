...
public class PostProcessor {
    ...
    public static void postProcess(MinecraftWorld minecraftWorld, Rectangle area, ProgressReceiver progressReceiver) throws ProgressReceiver.OperationCancelled {
        if(enabled) {
            ...
            /**
             * This will only execute if enforceBlockRules is set to false, but fixDirt is
             * set to true. This will ignore fixes EXCEPT for dirt fixes.
             */
        } else if(fixDirt & !enabled) {
            if (progressReceiver != null) {
                progressReceiver.setMessage("Not enforcing Minecraft rules excecpt on dirt blocks.");
            }
            int x1 = area.x;
            int y1 = area.y;
            int x2 = x1 + area.width - 1, y2 = y1 + area.height - 1;
    ...
    private static final boolean fixDirt = ! "false".equalsIgnoreCase(System.getProperty("org.pepsoft.worldpainter.fixDirt"));
    ...
    static {
        ...
        if(!enabled & fixDirt) {
            logger.warn("Block rule enforcement disabled but will fix dirt");
        }
    }
}