...
public class DefaultColourRamp implements ColourRamp {
    public DefaultColourRamp(int minHeight, int waterLevel, int maxHeight) {
        ...
        if (minHeight < 0) {
            waterRamp.put((float) minHeight, new ColourGradient(minHeight, 0x000000, 0, 0x0000ff));
        }
        if (waterLevel > 0) {
            waterRamp.put(0.0f, new ColourGradient(0.0f, 0x0000ff, waterLevel, 0x00ffff));
        }
        final int step = (256 - waterLevel) / 5;
        ...
        for (int i = 0; i < 5; i++) {
            ...
            landRamp.put((i > 0) ? (float) (waterLevel + i * step) : -Float.MAX_VALUE, new ColourGradient(waterLevel + i * step, colour, (i < 4) ? waterLevel + (i + 1) * step : 255, nextColour));
            colour = nextColour;
        }
        if (maxHeight > 256) {
            landRamp.put(255.0f, new ColourGradient(255, 0xffffff, maxHeight, 0xff7fff)); // purple
        }
    }
